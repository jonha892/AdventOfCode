class Day11: Day {
  ...
  func manhattenDist2(
    from start: Coordinate2D, to end: Coordinate2D, in grid: [[Character]], _ factor: Int
  ) -> Int {
    var extendRowIdx: [Int] = []
    var extendColIdx: [Int] = []
    ...
    return abs(start.x - end.x) + extendColIdx.count * (factor - 1) + abs(start.y - end.y)
      + extendRowIdx
      .count * (factor - 1)
  }
  ...
  func partOne() -> Int {
    ...
    let galaxyPositions: [Coordinate2D] = grid.enumerated().flatMap {
      (rowIdx, row) -> [Coordinate2D] in
  ...
  func partTwo() -> Int {
    ...
    for (i, (start, end)) in combinations.enumerated() {
      //let distance = walk(from: start, to: end, in: grid)
      let distance = manhattenDist2(from: start, to: end, in: self.grid, 1_000_000)
      print("(\(i+1) / \(combinations.count)) distance: \(distance)")
      sum += distance
    }
